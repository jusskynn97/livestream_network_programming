...
export default function Page() {
    ...
    const isBanned = user_id && stream?.settings?.banned_users?.includes(user_id);
        ...
        const fetchStream = async () => {
            try {
                ...
                await client.collection('streams').subscribe(stream.id, (new_stream) => {
                    setStream((prev: any) => ({ ...prev, ...new_stream.record }));
                });
            } catch (error) {
                setStream({
                    exists: false
    ...
    return (
        <DefaultLayout>
            ...
            <div className="p-6 flex flex-col">
                ...
                {stream !== null && stream?.exists !== false && !isBanned && (
                    <>
                        <div className="flex flex-col lg:flex-row lg:space-x-4">
                            ...
                            <div className="w-full mt-4 lg:w-1/4 lg:mt-0">
                                <StreamChat stream={stream} stream_key={stream_key ?? null} stream_id={stream?.id} stream_settings={stream?.settings ?? {}} />
                            </div>
                        </div>
                ...
                {stream !== null && stream?.exists !== false && isBanned && (
                    <div className="flex flex-col justify-center items-center h-96 space-y-4">
                        <h1 className="text-4xl font-bold text-destructive">You have been banned</h1>
                        <p className="text-xl text-muted-foreground">You cannot watch this stream or participate in the chat.</p>
                    </div>
                )}
            </div>
        </DefaultLayout>
