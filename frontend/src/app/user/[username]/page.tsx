...
import { Bell, Eye, Heart, Play } from "lucide-react";
...
export default function Page() {
    ...
    const checkIfFollowing = async (targetUserId: string) => {
        if (!authUser) return;
        try {
            const records = await client.collection('followers').getList(1, 1, {
                filter: `follower="${authUser.id}" && following="${targetUserId}"`
            });
            setIsFollowing(records.items.length > 0);
        } catch (e) {
            console.error(e);
        }
    }
    ...
    const handleFollow = async () => {
        ...
        try {
            if (isFollowing) {
                // Unfollow
                const records = await client.collection('followers').getList(1, 1, {
                    filter: `follower="${authUser.id}" && following="${user.id}"`
                });
                if (records.items.length > 0) {
                    await client.collection('followers').delete(records.items[0].id);
                    setIsFollowing(false);
                    setUser((prev: any) => ({ ...prev, followers: (prev.followers || 1) - 1 }));
                }
            } else {
                // Follow
                await client.collection('followers').create({
                ...
                if (user.id !== authUser.id) {
                    await client.collection('notifications').create({
                        ...
                        is_read: false,
                        related_id: `/user/${authUser.username}`
                    });
                }

                setIsFollowing(true);
                setUser((prev: any) => ({ ...prev, followers: (prev.followers || 0) + 1 }));
            }
        } catch (e) {
    ...
    useEffect(() => {
        ...
        const fetchRecordings = async () => {
            try {
                const res = await client.collection('recordings').getList(1, 20, {
                    filter: `user="${user.id}"`,
                    sort: '-created',
                    expand: 'stream',
                });
                ...
                <section className="w-full h-full z-10 relative px-10 py-6 flex flex-col space-y-12">
                        ...
                        {isClient && user ? (
                            <>
                                ...
                                <div className="flex flex-row space-x-2 mt-2 lg:mt-0">
                                    ...
                                    <Button type="button" variant="accent" onClick={handleFollow}>
                    ...
                    <Tabs defaultValue="home" className="w-full !mt-0 lg:!mt-4">
                        ...
                        <TabsContent value="videos" className="mt-0">
                            <div className="flex flex-col bg-muted rounded-tl-none rounded-xl p-4">
                                <h1 className="font-bold text-xl text-white">Past Broadcasts</h1>
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-2">
                                    {recordings.length === 0 ? (
                                        <p className="text-gray-400">No past broadcasts found.</p>
                                    ) : (
                                        recordings.map((rec) => {
                                            const thumbnail = rec.thumbnail 
                                                ? client.files.getUrl(rec, rec.thumbnail) 
                                                : (rec.expand?.stream?.thumbnail 
                                                    ? client.files.getUrl(rec.expand.stream, rec.expand.stream.thumbnail) 
                                                    : null);
                                            
                                            return (
                                                <div key={rec.id} className="relative w-full aspect-video flex flex-col bg-black/20 rounded-xl overflow-hidden cursor-pointer hover:ring-2 ring-accent transition-all" onClick={() => window.location.href = `/watch/${rec.id}`}>
                                                    {thumbnail ? (
                                                        <Image src={thumbnail} alt={rec.title} layout="fill" objectFit="cover" />
                                                    ) : (
                                                        <div className="w-full h-full flex items-center justify-center bg-gray-800">
                                                            <Play size={48} className="text-white/50" />
                                                        </div>
                                                    )}
                                                    <div className="absolute bottom-0 left-0 w-full p-2 bg-gradient-to-t from-black/80 to-transparent z-10">
                                                        <p className="font-bold truncate text-white">{rec.title}</p>
                                                        <p className="text-xs text-gray-300">{new Date(rec.created).toLocaleString()}</p>
                                                    </div>
                                                </div>
                                            )
                                        })
