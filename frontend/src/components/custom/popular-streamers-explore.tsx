    ...
    useEffect(() => {
        async function fetchStreams() {
            ...
            const data = res.items.map((item: any) => {
                ...
                const avatar = client.files.getUrl(item.expand?.user, item.expand?.user?.avatar);
                return { ...item, thumbnail: img, avatarUrl: avatar };
            });
                        ...
                        <div className="flex flex-col xl:flex-row space-x-4 w-full">
                            ...
                            <div className="w-full xl:w-3/5 h-auto">
                                <CustomStreamPreview
                                    ...
                                    avatarUrl={streams && streams[0]?.avatarUrl}
                                />
                            </div>
...
function CustomStreamPreview(
    { title, viewers, isLive, streamer, game, image, stream_id, avatarUrl }: StreamPreviewProps
) {
